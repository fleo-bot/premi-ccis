...
public class RequestInboxDAO {
    ...
    public static void addToInbox(RoomRequest request) {
        ...
        String sql = "INSERT INTO inbox_requests (user_id, room_name, booking_date, start_time, end_time, status, full_name, professor_name, program, section, year) VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?)";
        try (Connection conn = DBConnection.getConnection();
            ...
            stmt.setString(6, request.getFullName());
            stmt.setString(7, request.getProfessorName());
            stmt.setString(8, request.getProgram());
            stmt.setString(9, request.getSection());
            stmt.setInt(10, request.getYear());
            stmt.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static List<RoomRequest> getUserInbox(int userId) {
        ...
        try (Connection conn = DBConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                request.setProfessorName(rs.getString("professor_name"));
                request.setProgram(rs.getString("program"));
                request.setSection(rs.getString("section"));
                request.setYear(rs.getInt("year"));
                list.add(request);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static boolean bookRoomDirectly(RoomRequest request) {
        try (Connection conn = DBConnection.getConnection()) {
            String sql = "INSERT INTO bookings (user_id, room_name, booking_date, start_time, end_time, full_name, professor_name, program, section, year) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setInt(1, request.getUserId());
            stmt.setString(2, request.getChosenRoom());
            stmt.setDate(3, request.getBookingDate());
    ...
    public static List<Map<String, Object>> getAllCurrentBookings() {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                booking.put("professorName", rs.getString("professor_name"));
                booking.put("program", rs.getString("program"));
                booking.put("section", rs.getString("section"));
                booking.put("year", rs.getInt("year"));
    ...
    public static boolean approveAndBook(RoomRequest request) {
        try (Connection conn = DBConnection.getConnection()) {
            ...
            String bookSql = "INSERT INTO bookings (user_id, room_name, booking_date, start_time, end_time, full_name, professor_name, program, section, year) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            bookStmt.setString(6, request.getFullName());
            bookStmt.setString(7, request.getProfessorName());
            bookStmt.setString(8, request.getProgram());
            bookStmt.setString(9, request.getSection());
            bookStmt.setInt(10, request.getYear());
    ...
    public static RoomRequest getRequestById(int requestId) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                req.setProfessorName(rs.getString("professor_name"));
                req.setProgram(rs.getString("program"));
                req.setSection(rs.getString("section"));
                req.setYear(rs.getInt("year"));

                // Set defaults if null
                if (req.getFullName() == null) req.setFullName("Not Provided");
                if (req.getProfessorName() == null) req.setProfessorName("Not Provided");
                if (req.getProgram() == null) req.setProgram("BSIT");
                if (req.getSection() == null) req.setSection("Unknown");
                if (req.getYear() == 0) req.setYear(0);
