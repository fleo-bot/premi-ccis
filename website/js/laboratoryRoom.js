...
window.addEventListener('DOMContentLoaded', function () {
  ...
  proceedBtn.addEventListener('click', async function (e) {
    ...
    const sectionYear = sectionYearInput.value;
    const sectionYearParts = sectionYear.split('-').map(s => s.trim());
    if (sectionYearParts.length !== 2 || !sectionYearParts[0] || !sectionYearParts[1]) {
      resultsContainer.textContent = 'Please enter section and year in the format "Section-Year" e.g., "A-1".';
      return;
    }
    const [section, year] = sectionYearParts;

    if (!students || !pcs || !date || !time || !duration || !fullName || !professorName || !program || !section || !year) {
